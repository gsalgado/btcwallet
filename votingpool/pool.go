...
// exist.
func (p *Pool) GetSeries(seriesID uint32) *SeriesData {
...
func (p *Pool) CreateSeries(version, seriesID, reqSigs uint32, rawPubKeys []string) error {
	if series := p.GetSeries(seriesID); series != nil {
...
func (p *Pool) addressFor(script []byte) (btcutil.Address, error) {
	...
	return btcutil.NewAddressScriptHashFromHash(scriptHash, p.manager.Net())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	for i, key := range pubKeys {
		...
		pks[i], err = btcutil.NewAddressPubKey(pubkey.SerializeCompressed(), p.manager.Net())
		if err != nil {
			str := fmt.Sprintf(
...
func (p *Pool) WithdrawalAddress(
	seriesID uint32, branch Branch, index Index) (*WithdrawalAddress, error) {
	...
	if addr == nil {
		str := fmt.Sprintf(
			"cannot withdraw from unused address (series: %d, branch: %d, index: %d)",
...
func (p *Pool) poolAddress(
	seriesID uint32, branch Branch, index Index, script []byte) (*poolAddress, error) {
...
func (p *Pool) getUsedAddr(
	seriesID uint32, branch Branch, index Index) (waddrmgr.ManagedScriptAddress, error) {
	...
	addr, err := btcutil.NewAddressScriptHashFromHash(hash, mgr.Net())
	if err != nil {
